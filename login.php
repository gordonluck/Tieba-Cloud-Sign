...
require SYSTEM_ROOT.'/lib/class.login.php';
global $m;
...
if($_GET['do']=='sendcode'){
	$array=$login->sendcode($_POST['type'],$_POST['lstr'],$_POST['ltoken']);
}
if($_GET['do']=='login'){
	$array=$login->login($_POST['time'],$_POST['user'],$_POST['pwd'],$_POST['p'],$_POST['vcode'],$_POST['vcodestr']);
}
if($_GET['do']=='login2'){
	$array=$login->login2($_POST['type'],$_POST['lstr'],$_POST['ltoken'],$_POST['vcode']);
}
if($_GET['do']=='getvcpic'){
...
if(!empty($array['bduss'])) {
$m->insert("baiduid",Array('name'=>$array['displayname'],'bduss'=>$array['bduss'],'uid'=>UID));}